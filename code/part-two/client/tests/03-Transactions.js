...
describe("Transactions module", function() {
  describe("createTransaction", function() {
    ...
    it("should return a Transaction with the correct properties", function() {
      expect(transaction.header).to.be.set.and.be.bytes;
      expect(transaction.headerSignature).to.be.set.and.be.a.hexString;
      expect(transaction.payload).to.be.set.and.be.bytes;
    });
    ...
    it("should include TransactionHeader with correct properties", function() {
      ...
      expect(header.signerPublicKey).to.be.set.and.be.a.hexString.and.equal(
        keys.publicKey
      );
      expect(header.batcherPublicKey).to.be.set.and.be.a.hexString.and.equal(
        keys.publicKey
      );
      expect(header.familyName)
        .to.be.set.and.be.a("string")
        .and.equal("cryptomoji");
      expect(header.familyVersion)
        .to.be.set.and.be.a("string")
        .and.equal("0.1");

      expect(header.inputs).to.be.set.and.be.an("array");
      header.inputs.forEach(input => {
        expect(input).to.be.a.hexString.and.match(/^5f4d76/);
      });
      expect(header.outputs).to.be.set.and.be.an("array");
  ...
  describe("createBatch", function() {
    ...
    it("should return a Batch with the correct properties", function() {
      expect(batch.header).to.be.set.and.be.bytes;
      expect(batch.headerSignature).to.be.set.and.be.a.hexString;
      expect(batch.transactions).to.be.set.and.be.an("array");
    });
    ...
    it("should create a Batch with multiple transactions", function() {
      ...
      expect(batch.transactions).to.deep.equal(transactionArray);
      expect(header.transactionIds).to.deep.equal(
        transactionArray.map(t => t.headerSignature)
      );
    });
  });
  ...
  describe("encodeAll", function() {
    ...
    it("should encode multiple payloads", function() {
      ...
      const decodedPayloads = BatchList.decode(
        encoded
      ).batches[0].transactions.map(t => decode(t.payload));
